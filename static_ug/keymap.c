...
// 32x42 Fibonacci spiral (squares + arcs, Wikipedia-style), row-major MSB-first
#define FIB_SPIRAL_H 42
static const uint8_t PROGMEM fib_spiral_pixels[168] = {
    0x1F, 0xFF, 0xFF, 0xF8,
    0x10, 0x0C, 0x00, 0x08,
    0x10, 0x0C, 0x00, 0x08,
    0x10, 0x0C, 0x00, 0x08,
...
static void render_slave(void) {
    ...
    // Fibonacci spiral centered in the lower half (y=59-100, 42 px tall)
    for (uint8_t row = 0; row < FIB_SPIRAL_H; row++) {
        for (uint8_t col_byte = 0; col_byte < 4; col_byte++) {
            uint8_t byte = pgm_read_byte(&fib_spiral_pixels[row * 4 + col_byte]);
            for (uint8_t bit = 0; bit < 8; bit++) {
                uint8_t x = col_byte * 8 + bit;
                bool on = (byte & (1 << (7 - bit))) != 0;
                oled_write_pixel(x, 59 + row, on);
            }
        }
    }
}
